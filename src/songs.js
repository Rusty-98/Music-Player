export const INITIAL_SONGS = [
  {
    id: 1,
    title: "Metamorphosis",
    artist: "INTERWORLD",
    album: "Metamorphosis",
    genre: "Phonk",
    year: "2021",
    src: "./1.mp3",
    cover: "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=600&auto=format&fit=crop&q=80",
    accentColor: "#a855f7",
    lyrics: [
      { time: 0, text: "♪ [Dark Drift Phonk Intro] ♪" },
      { time: 8, text: "Feel the rhythm in the dark" },
      { time: 16, text: "Metamorphosis taking over the mind" },
      { time: 24, text: "Heavy bass rolling through the night" },
      { time: 32, text: "Drifting in neon shadows" },
      { time: 45, text: "♪ [Bass Drop & Synth Solo] ♪" },
      { time: 60, text: "Speeding past the horizon" },
      { time: 75, text: "Echoes of the underground" }
    ]
  },
  {
    id: 2,
    title: "Rapture",
    artist: "INTERWORLD",
    album: "Rapture Phonk",
    genre: "Phonk",
    year: "2022",
    src: "./2.mp3",
    cover: "https://images.unsplash.com/photo-1579783902614-a3fb3927b675?w=600&auto=format&fit=crop&q=80",
    accentColor: "#ec4899",
    lyrics: [
      { time: 0, text: "♪ [Atmospheric Cowbell Build] ♪" },
      { time: 12, text: "Caught in the rapture of sound" },
      { time: 22, text: "Distorted kicks pulsing in the core" },
      { time: 35, text: "No looking back once the bass hits" },
      { time: 48, text: "♪ [Phonk Cowbell Melodies] ♪" },
      { time: 65, text: "High RPM in the midnight run" }
    ]
  },
  {
    id: 3,
    title: "Starboy",
    artist: "The Weeknd ft. Daft Punk",
    album: "Starboy",
    genre: "Pop / R&B",
    year: "2016",
    src: "./3.mp3",
    cover: "https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=600&auto=format&fit=crop&q=80",
    accentColor: "#ef4444",
    lyrics: [
      { time: 0, text: "♪ [Electronic Piano Intro] ♪" },
      { time: 10, text: "I'm tryna put you in the worst mood, ah" },
      { time: 15, text: "P1 cleaner than your church shoes, ah" },
      { time: 20, text: "Milli point two just to hurt you, ah" },
      { time: 25, text: "All red Lamb' just to tease you, ah" },
      { time: 30, text: "None of these toys on lease too, ah" },
      { time: 35, text: "Look what you've done" },
      { time: 38, text: "I'm a motherfuckin' starboy" },
      { time: 45, text: "Look what you've done, I'm a starboy" }
    ]
  },
  {
    id: 4,
    title: "Cheques",
    artist: "Shubh",
    album: "Still Rollin",
    genre: "Punjabi",
    year: "2023",
    src: "./4.mp3",
    cover: "https://images.unsplash.com/photo-1508700115892-45ecd05ae2ad?w=600&auto=format&fit=crop&q=80",
    accentColor: "#f59e0b",
    lyrics: [
      { time: 0, text: "♪ [Smooth Trap Beat] ♪" },
      { time: 8, text: "Saddi dekh ke chadhayi vairi darde" },
      { time: 15, text: "Jehde kehnde si ki kuch ni ban sakda" },
      { time: 22, text: "Ajj sign cheques hunde lakh lakh de" },
      { time: 30, text: "Gallan sachiyan te rob poora rakhde" },
      { time: 42, text: "Kardi mandeer follow sada style ni" },
      { time: 55, text: "Top di aan taur naal ghumde" }
    ]
  },
  {
    id: 5,
    title: "Diggy (Jim Do)",
    artist: "Lo-Fi Collective",
    album: "Chill Study Beats",
    genre: "Lo-Fi / Chill",
    year: "2023",
    src: "./5.mp3",
    cover: "https://images.unsplash.com/photo-1518709268805-4e9042af9f23?w=600&auto=format&fit=crop&q=80",
    accentColor: "#06b6d4",
    lyrics: [
      { time: 0, text: "♪ [Warm Vinyl Crackle & Rain FX] ♪" },
      { time: 12, text: "Deep breaths, gentle rain outside..." },
      { time: 24, text: "Focus, unwind, and let thoughts drift" },
      { time: 40, text: "♪ [Mellow Rhodes Chords & Subtle Beats] ♪" },
      { time: 60, text: "Late night coffee & soothing melodies" },
      { time: 80, text: "Peace in every frequency" }
    ]
  },
  {
    id: 6,
    title: "Unravel",
    artist: "TK from Ling Tosite Sigure",
    album: "Tokyo Ghoul OST",
    genre: "Anime",
    year: "2014",
    src: "./6.mp3",
    cover: "https://images.unsplash.com/photo-1534447677768-be436bb09401?w=600&auto=format&fit=crop&q=80",
    accentColor: "#6366f1",
    lyrics: [
      { time: 0, text: "Oshiete oshiete yo sono shikumi wo" },
      { time: 8, text: "Boku no naka ni dare ga iru no?" },
      { time: 16, text: "Kowareta kowareta yo kono sekai de" },
      { time: 24, text: "Kimi ga warau nanimo miezu ni" },
      { time: 32, text: "♪ [Explosive Guitar & Drum Solo] ♪" },
      { time: 48, text: "Kowareta boku nante sa iki wo tomete" },
      { time: 58, text: "Hodokenai mou hodokenai yo shinjitsu sae" },
      { time: 70, text: "Freeze!" },
      { time: 76, text: "Unravel ghoul..." }
    ]
  },
  {
    id: 7,
    title: "Patola",
    artist: "Guru Randhawa ft. Bohemia",
    album: "Page One",
    genre: "Punjabi",
    year: "2015",
    src: "./7.mp3",
    cover: "https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=600&auto=format&fit=crop&q=80",
    accentColor: "#10b981",
    lyrics: [
      { time: 0, text: "♪ [Bohemia Rap Intro & Dhol Beats] ♪" },
      { time: 10, text: "Suit patiala tera jutti sharabi" },
      { time: 18, text: "Lagdi patola tu lagdi punjabi" },
      { time: 26, text: "Ve munda darda na tera naah lenda" },
      { time: 34, text: "Ni kudiye tu lagdi patola" },
      { time: 45, text: "♪ [Dholak Groove & Synthesizer] ♪" }
    ]
  },
  {
    id: 8,
    title: "Itachi Theme (Senya)",
    artist: "Yasuharu Takanashi",
    album: "Naruto Shippuden OST",
    genre: "Anime",
    year: "2008",
    src: "./8.mp3",
    cover: "https://images.unsplash.com/photo-1563089145-599997674d42?w=600&auto=format&fit=crop&q=80",
    accentColor: "#dc2626",
    lyrics: [
      { time: 0, text: "♪ [Mournful Shakuhachi Flute] ♪" },
      { time: 15, text: "The moon turns crimson over the Leaf Village" },
      { time: 30, text: "Carrying the weight of eternal sacrifice" },
      { time: 45, text: "♪ [Emotional Strings & Choral Harmony] ♪" },
      { time: 65, text: "A hero who lived and died in darkness" }
    ]
  },
  {
    id: 9,
    title: "Timeless",
    artist: "The Weeknd & Playboi Carti",
    album: "Hurry Up Tomorrow",
    genre: "Pop / R&B",
    year: "2024",
    src: "./YTDown.com_YouTube_The-Weeknd-Playboi-Carti-Timeless_Media_5EpyN_6dqyk_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=600&auto=format&fit=crop&q=80",
    accentColor: "#3b82f6",
    lyrics: [
      { time: 0, text: "♪ [Hypnotic Synth Arpeggio] ♪" },
      { time: 12, text: "Ever since I was a jit, I knew I was the shit" },
      { time: 22, text: "Shorty pull up with the clique" },
      { time: 32, text: "Timeless, we don't follow trends" },
      { time: 45, text: "City lights glow in the rearview mirror" }
    ]
  },
  {
    id: 10,
    title: "One Of The Girls",
    artist: "The Weeknd, JENNIE, Lily-Rose Depp",
    album: "The Idol OST",
    genre: "Pop / R&B",
    year: "2023",
    src: "./YTDown.com_YouTube_The-Weeknd-JENNIE-Lily-Rose-Depp-One-Of-_Media_Mx92lTYxrJQ_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1520523839898-50712825e3a7?w=600&auto=format&fit=crop&q=80",
    accentColor: "#d946ef",
    lyrics: [
      { time: 0, text: "♪ [Sensual Synth Bassline] ♪" },
      { time: 10, text: "Lock me up and throw away the key" },
      { time: 20, text: "He knows how to make me feel right" },
      { time: 32, text: "Give me tough love, leave me breathless" },
      { time: 48, text: "Tell nobody I'm your secret obsession" }
    ]
  },
  {
    id: 11,
    title: "I Really Do",
    artist: "Karan Aujla x Ikky",
    album: "Four You",
    genre: "Punjabi",
    year: "2023",
    src: "./YTDown.com_YouTube_I-REALLY-DO-MUSIC-VIDEO-KARAN-AUJLA-IKKY_Media_K9R7KcaettM_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1518609878373-06d740f60d8b?w=600&auto=format&fit=crop&q=80",
    accentColor: "#f97316",
    lyrics: [
      { time: 0, text: "♪ [Ikky Beats Guitar Riff] ♪" },
      { time: 9, text: "Ikky! Karan Aujla!" },
      { time: 16, text: "Mainu pata tainu mere naal pyar ae" },
      { time: 24, text: "Karan jo vi gallan sidha dilon kardaa" },
      { time: 36, text: "I really do love the way you move" }
    ]
  },
  {
    id: 12,
    title: "Jee Ni Lagda",
    artist: "Karan Aujla",
    album: "Making Memories",
    genre: "Punjabi",
    year: "2023",
    src: "./YTDown.com_YouTube_Jee-Ni-Lagda-Full-Video-Karan-Aujla-I-Ma_Media_BXNxrT59MzQ_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=600&auto=format&fit=crop&q=80",
    accentColor: "#eab308",
    lyrics: [
      { time: 0, text: "♪ [Acoustic Guitar Melodies] ♪" },
      { time: 12, text: "Tere bina mera jee ni lagda" },
      { time: 22, text: "Raatan nu vi chain naio labhda" },
      { time: 34, text: "Kite mil jawe tu supne ch aake" },
      { time: 46, text: "Dass de tu kyon door ho gayi" }
    ]
  },
  {
    id: 13,
    title: "H.I.M",
    artist: "Karan Aujla x Ikky",
    album: "Street Dreams",
    genre: "Punjabi",
    year: "2024",
    src: "./YTDown.com_YouTube_Karan-Aujla-Ikky-HIM-Official-Audio_Media_eoYsV5RwyA_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1509198397868-475647b2a1e5?w=600&auto=format&fit=crop&q=80",
    accentColor: "#ea580c",
    lyrics: [
      { time: 0, text: "♪ [Ikky Heavy Bassline] ♪" },
      { time: 8, text: "They know that I'm HIM" },
      { time: 16, text: "Karan Aujla rule the streets" },
      { time: 28, text: "Karde talk te karde walk" },
      { time: 42, text: "Worldwide recognition!" }
    ]
  },
  {
    id: 14,
    title: "You're U Tho (You)",
    artist: "Karan Aujla x Ikky",
    album: "Four You",
    genre: "Punjabi",
    year: "2023",
    src: "./YTDown.com_YouTube_Karan-Aujla-Ikky-You-re-U-Tho-Official-A_Media_xciwYxZuxog_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1518609878373-06d740f60d8b?w=600&auto=format&fit=crop&q=80",
    accentColor: "#d97706",
    lyrics: [
      { time: 0, text: "♪ [Smooth Melodic R&B Beat] ♪" },
      { time: 10, text: "You're you though, nobody can replace you" },
      { time: 20, text: "Dil de kareeb tu hi rehan wali ae" },
      { time: 35, text: "Kite jaavin na chhad ke mainu" }
    ]
  },
  {
    id: 15,
    title: "White Brown Black",
    artist: "Avvy Sra x Karan Aujla x Jaani",
    album: "White Brown Black",
    genre: "Punjabi",
    year: "2022",
    src: "./YTDown.com_YouTube_White-Brown-Black-Avvy-Sra-Karan-Aujla-J_Media_BtQp2U6hJII_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1508700115892-45ecd05ae2ad?w=600&auto=format&fit=crop&q=80",
    accentColor: "#8b5cf6",
    lyrics: [
      { time: 0, text: "♪ [Upbeat Club Dhol & Synth] ♪" },
      { time: 8, text: "White Brown Black mundeyan da tola" },
      { time: 18, text: "Avvy Sra beats with Karan Aujla" },
      { time: 30, text: "Jaani diyan likhiyan gallan sachiyan" }
    ]
  },
  {
    id: 16,
    title: "Shkini",
    artist: "Guru Randhawa",
    album: "Shkini",
    genre: "Punjabi",
    year: "2024",
    src: "./YTDown.com_YouTube_GURU-RANDHAWA-SHKINI-MV_Media_4RSxIX2I6Ic_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=600&auto=format&fit=crop&q=80",
    accentColor: "#e11d48",
    lyrics: [
      { time: 0, text: "♪ [Bhangra Pop Intro] ♪" },
      { time: 10, text: "Paunda shkini munda top di" },
      { time: 20, text: "Guru Randhawa brand new track" },
      { time: 32, text: "Nachdi dunia saadi dhun te" }
    ]
  },
  {
    id: 17,
    title: "Kalyani",
    artist: "Shreya Ghoshal, ARJN, KDS",
    album: "Kalyani",
    genre: "Indie / Classical",
    year: "2024",
    src: "./YTDown.com_YouTube_KALYANI-with-Shreya-Ghoshal-ARJN-KDS-FIF_Media_z5y8Clp_TdE_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1465847899084-d164df4dedc6?w=600&auto=format&fit=crop&q=80",
    accentColor: "#f43f5e",
    lyrics: [
      { time: 0, text: "♪ [Soulful Sitar & Classical Vocal Aalap] ♪" },
      { time: 12, text: "Kalyani raaga in modern symphony" },
      { time: 25, text: "Suran di mehak te Shreya di aawaz" },
      { time: 40, text: "Soul stirring harmonies floating high" }
    ]
  },
  {
    id: 18,
    title: "Pavazha Malli",
    artist: "Sai Abhyankkar",
    album: "Pavazha Malli",
    genre: "Indie / Pop",
    year: "2024",
    src: "./YTDown.com_YouTube_SaiAbhyankkar-Pavazha-Malli-Music-Video_Media_b68HETiNO98_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=600&auto=format&fit=crop&q=80",
    accentColor: "#06b6d4",
    lyrics: [
      { time: 0, text: "♪ [Catchy Acoustic Guitars & Indie Rhythm] ♪" },
      { time: 10, text: "Pavazha malli poove poove" },
      { time: 22, text: "Fresh vibes from Sai Abhyankkar" },
      { time: 35, text: "Groove to the indie melodic beats" }
    ]
  },
  {
    id: 19,
    title: "Samne Rehni E",
    artist: "Pav Dharia",
    album: "SOLO",
    genre: "Punjabi",
    year: "2023",
    src: "./YTDown.com_YouTube_Samne-Rehni-E-Full-Video-SOLO-Pav-Dharia_Media_rrlY04Hmb4U_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=600&auto=format&fit=crop&q=80",
    accentColor: "#10b981",
    lyrics: [
      { time: 0, text: "♪ [Romantic Acoustic Piano & Pav Dharia Vocals] ♪" },
      { time: 12, text: "Har pal tu samne rehni e" },
      { time: 24, text: "Mere dil vich vasdi tu" },
      { time: 38, text: "Pav Dharia signature melody" }
    ]
  },
  {
    id: 20,
    title: "Bounce",
    artist: "Shubh",
    album: "Leo",
    genre: "Punjabi",
    year: "2024",
    src: "./YTDown.com_YouTube_Shubh-Bounce-Official-Music-Video_Media_UX740Wf7OUc_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?w=600&auto=format&fit=crop&q=80",
    accentColor: "#14b8a6",
    lyrics: [
      { time: 0, text: "♪ [Heavy 808 Drill Bass] ♪" },
      { time: 8, text: "Yeah, let it bounce!" },
      { time: 16, text: "Gaddi ch baitho te suno mera flow" },
      { time: 26, text: "Worldwide chale sadda gaana bro" },
      { time: 38, text: "Koyi saade agge tikda nahi" }
    ]
  },
  {
    id: 21,
    title: "Dracula",
    artist: "Tame Impala",
    album: "Psychedelic Waves",
    genre: "Indie / Psych",
    year: "2023",
    src: "./YTDown.com_YouTube_Tame-Impala-Dracula-Lyrics_Media_HWyzKmXquJk_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1507676184212-d03ab07a01bf?w=600&auto=format&fit=crop&q=80",
    accentColor: "#8b5cf6",
    lyrics: [
      { time: 0, text: "♪ [Dreamy Flanger Synth Chord] ♪" },
      { time: 14, text: "Drifting in the vintage haze" },
      { time: 26, text: "Echoes of psychedelic days" },
      { time: 40, text: "Is it real or just imagination?" },
      { time: 55, text: "Dracula lurking in the twilight reverie" }
    ]
  },
  {
    id: 22,
    title: "Yezdi",
    artist: "Nanku",
    album: "Indie Odyssey",
    genre: "Indie / Chill",
    year: "2023",
    src: "./YTDown.com_YouTube_Nanku-Yezdi-Official-Visualiser-2023_Media_RzR1-1SQ25s_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1558981403-c5f9899a28bc?w=600&auto=format&fit=crop&q=80",
    accentColor: "#0ea5e9",
    lyrics: [
      { time: 0, text: "♪ [Warm Acoustic Lofi Vibe] ♪" },
      { time: 10, text: "Sawaar yezdi te hawa naal gallan" },
      { time: 22, text: "Dil nu sakoon te manzil da na pata" },
      { time: 35, text: "Khaab saare sach honge ek din" }
    ]
  },
  {
    id: 23,
    title: "SIX (Sirra)",
    artist: "Guru Randhawa",
    album: "SIX",
    genre: "Punjabi",
    year: "2024",
    src: "./YTDown.com_YouTube_GURU-RANDHAWA-SIX-MV_Media_PEdDI1A17qc_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f?w=600&auto=format&fit=crop&q=80",
    accentColor: "#f43f5e",
    lyrics: [
      { time: 0, text: "♪ [High Energy Club Synth Intro] ♪" },
      { time: 10, text: "Check one two, Guru is back in the zone" },
      { time: 20, text: "Hit six on every track" },
      { time: 32, text: "Turn up the bass to maximum" }
    ]
  },
  {
    id: 24,
    title: "Under The Influence x I Was Never There",
    artist: "Chris Brown & The Weeknd",
    album: "Viral Phonk Remix",
    genre: "Phonk",
    year: "2023",
    src: "./YTDown.com_YouTube_Under-The-Influence-x-I-Was-Never-There-_Media_9eIhjXZcwyY_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1509198397868-475647b2a1e5?w=600&auto=format&fit=crop&q=80",
    accentColor: "#8b5cf6",
    lyrics: [
      { time: 0, text: "♪ [Slowed Reverb Dark Synth] ♪" },
      { time: 12, text: "Trust me, girl, you know I'm under the influence" },
      { time: 25, text: "When it's said and done, I was never there" },
      { time: 40, text: "Midnight vibes and fading memories" },
      { time: 55, text: "♪ [Heavy Phonk Bass Drop] ♪" }
    ]
  },
  {
    id: 25,
    title: "Baby Girl",
    artist: "Guru Randhawa & Dhvani Bhanushali",
    album: "Baby Girl",
    genre: "Punjabi",
    year: "2020",
    src: "./YTDown.com_YouTube_Baby-Girl-Guru-Randhawa-Dhvani-Bhanushal_Media_pLhNdJNwGC8_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?w=600&auto=format&fit=crop&q=80",
    accentColor: "#ec4899",
    lyrics: [
      { time: 0, text: "♪ [Bhangra Pop Beat Intro] ♪" },
      { time: 8, text: "Kudiye ni tere brown rang ne" },
      { time: 18, text: "Munde saare kill kar dite" },
      { time: 28, text: "Baby girl you look so fine" },
      { time: 40, text: "Grooving with Guru & Dhvani" }
    ]
  },
  {
    id: 26,
    title: "Dopamine",
    artist: "Guru Randhawa",
    album: "Dopamine",
    genre: "Punjabi",
    year: "2024",
    src: "./YTDown.com_YouTube_GURU-RANDHAWA-DOPAMINE-MV_Media_iOgR7hi90Ac_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=600&auto=format&fit=crop&q=80",
    accentColor: "#06b6d4",
    lyrics: [
      { time: 0, text: "♪ [High Energy Electronic Synth] ♪" },
      { time: 8, text: "Dopamine hit jadon tainu vekheya" },
      { time: 18, text: "Dil mera fast fast beat karda" },
      { time: 30, text: "Guru Randhawa brand new rush" },
      { time: 44, text: "Bass drop in the veins" }
    ]
  },
  {
    id: 27,
    title: "Kiya Kiya",
    artist: "Anand Raj Anand & Sunidhi Chauhan",
    album: "Welcome",
    genre: "Bollywood",
    year: "2007",
    src: "./YTDown.com_YouTube_Kiya-Kiya-Full-Audio-Welcome-Movie-Song-_Media_phcltUsSYbI_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=600&auto=format&fit=crop&q=80",
    accentColor: "#f59e0b",
    lyrics: [
      { time: 0, text: "♪ [Iconic Welcome Movie Groove Intro] ♪" },
      { time: 12, text: "Kiya kiya kya kiya sanam" },
      { time: 22, text: "Kiya kiya kya kiya sanam" },
      { time: 35, text: "Welcome to the party dance floor" },
      { time: 48, text: "♪ [Upbeat Bollywood Brass & Beats] ♪" }
    ]
  },
  {
    id: 28,
    title: "Na Kajre Ki Dhar",
    artist: "Pankaj Udhas & Sadhana Sargam",
    album: "Mohra",
    genre: "Bollywood",
    year: "1994",
    src: "./YTDown.com_YouTube_Na-Kajre-Ki-Dhar-With-Lyrics-Suniel-Shet_Media_C_wxaIOPMZk_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1465847899084-d164df4dedc6?w=600&auto=format&fit=crop&q=80",
    accentColor: "#10b981",
    lyrics: [
      { time: 0, text: "♪ [Melodious Flute & Santoor Intro] ♪" },
      { time: 14, text: "Na kajre ki dhar, na motiyon ke haar" },
      { time: 28, text: "Na koi kiya singaar, phir bhi kitni sundar ho" },
      { time: 45, text: "Tum kitni sundar ho..." },
      { time: 60, text: "♪ [Timeless Romantic Ghazal Melody] ♪" }
    ]
  },
  {
    id: 29,
    title: "Imaginary",
    artist: "Imran Khan",
    album: "Imaginary",
    genre: "Punjabi",
    year: "2015",
    src: "./YTDown.com_YouTube_Official-Music-Video-Imaginary-Imran-Kha_Media_eBI23_Sjfeo_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1518709268805-4e9042af9f23?w=600&auto=format&fit=crop&q=80",
    accentColor: "#6366f1",
    lyrics: [
      { time: 0, text: "♪ [Imran Khan Dutch Urban Beats] ♪" },
      { time: 10, text: "Let's go girl, let's fly away" },
      { time: 20, text: "Kakh vi na reha mere palle ni" },
      { time: 30, text: "You're my imaginary girl" },
      { time: 45, text: "Bass dropping worldwide" }
    ]
  },
  {
    id: 30,
    title: "Spain",
    artist: "Jassa Dhillon x Thiarajxtt",
    album: "VIBIN",
    genre: "Punjabi",
    year: "2023",
    src: "./YTDown.com_YouTube_Spain-Official-Video-with-Extended-Versi_Media_c20XsM9BWEM_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1508700115892-45ecd05ae2ad?w=600&auto=format&fit=crop&q=80",
    accentColor: "#ea580c",
    lyrics: [
      { time: 0, text: "♪ [Spanish Guitar x Heavy Punjabi 808 Trap] ♪" },
      { time: 10, text: "Spain walon flight seedhi aundi jatt di" },
      { time: 22, text: "Jassa Dhillon & Thiarajxtt on the beat" },
      { time: 34, text: "Vairi saare khunje laye hoye ne" },
      { time: 48, text: "♪ [Acoustic Spanish Riff & Drill Bass] ♪" }
    ]
  },
  {
    id: 31,
    title: "Yeh Haseen Vadiyan",
    artist: "A.R. Rahman, S.P.B & K.S. Chithra",
    album: "Roja",
    genre: "Bollywood",
    year: "1992",
    src: "./YTDown.com_YouTube_Yeh-Haseen-Vadiyan-Roja-A-R-Rahman-S-P-B_Media_5kZ5o-oM0RI_009_128k.mp3",
    cover: "https://images.unsplash.com/photo-1534447677768-be436bb09401?w=600&auto=format&fit=crop&q=80",
    accentColor: "#38bdf8",
    lyrics: [
      { time: 0, text: "♪ [Divine Mountain Symphony by A.R. Rahman] ♪" },
      { time: 15, text: "Yeh haseen vadiyan, yeh khula aasmaan" },
      { time: 28, text: "Aa gaye hum kahan, aye mere saajna" },
      { time: 42, text: "In baharon mein dil ki kali khil gayi" },
      { time: 58, text: "Mujhko tum jo mile, har khushi mil gayi..." }
    ]
  }
];
